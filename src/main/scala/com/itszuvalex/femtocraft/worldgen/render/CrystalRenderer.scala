...
import com.itszuvalex.femtocraft.render.BetterWavefrontObject
import com.itszuvalex.femtocraft.render.BetterObjModelLoader
...
class CrystalRenderer extends TileEntitySpecialRenderer {
  ...
  val crystalModel = AdvancedModelLoader.loadModel(CrystalRenderer.crystalModelLocation).asInstanceOf[BetterWavefrontObject]
  ...
  override def renderTileEntityAt(tile: TileEntity, x: Double, y: Double, z: Double, partialTime: Float): Unit = {
    ...
    (1 to 10).map(num => ("Gengon0" + (if (num < 10) "0" else "") + num, num)).foreach { name =>
      ...
      GL11.glColor4ub((color.red + colorOffset.red - 20).toByte, (color.green + colorOffset.green - 20).toByte, (color.blue + colorOffset.blue - 20).toByte, colorOffset.alpha)
