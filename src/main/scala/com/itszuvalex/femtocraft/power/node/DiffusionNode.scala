...
trait DiffusionNode extends PowerNode {
   ...
   */
  override def canAddParent(parent: IPowerNode) = DiffusionNode.PARENT_WHITELIST.contains(parent.getType)
   ...
   */
  override def canAddChild(child: IPowerNode) = DiffusionNode.CHILD_WHITELIST.contains(child.getType)
}
