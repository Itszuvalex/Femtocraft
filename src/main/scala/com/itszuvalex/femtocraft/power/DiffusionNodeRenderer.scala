...
class DiffusionNodeRenderer extends TileEntitySpecialRenderer {
  private val crystalModel = AdvancedModelLoader.loadModel(DiffusionNodeRenderer.crystalModelLocation)
  ...
  override def renderTileEntityAt(tile: TileEntity, x: Double, y: Double, z: Double, partialTime: Float): Unit = {
    tile match {
      case node: IPowerNode =>
        ...
        bindTexture(DiffusionNodeRenderer.crystalTexLocation)
