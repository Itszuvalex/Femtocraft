...
import com.itszuvalex.femtocraft.cyber.tile.{TileCondensationArray, TileCyberBase}
...
/**
  ...
  */
object MachineCondensationArray {
  val NAME = "Condensation Array"
}

class MachineCondensationArray extends ICyberMachine {
  /**
    * @return Name of the machine
    */
  override def getName = MachineCondensationArray.NAME
  ...
  override def multiblockRenderID: Int = RenderIDs.condensationArrayID
  ...
  override def formAtBaseAndIndex(world: World, baseController: TileCyberBase, machineIndex: Int): Unit = {
    ...
    getTakenLocations(world, mx, my, mz).foreach { loc =>
      world.setBlock(loc.x, loc.y, loc.z, FemtoBlocks.blockCondensationArray)
