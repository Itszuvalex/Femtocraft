...
import com.itszuvalex.femtocraft.cyber.tile.{TileCyberBase, TileCybermatDisintegrator}
...
class MachineCybermatDisintegrator extends ICyberMachine {
  ...
  override def formAtBaseAndIndex(world: World, baseController: TileCyberBase, machineIndex: Int): Unit = {
    ...
    getTakenLocations(world, mx, my, mz).foreach { loc =>
      world.setBlock(loc.x, loc.y, loc.z, FemtoBlocks.blockCybermatDisintegrator)
      world.getTileEntity(loc.x, loc.y, loc.z) match {
        case te: TileCybermatDisintegrator =>
