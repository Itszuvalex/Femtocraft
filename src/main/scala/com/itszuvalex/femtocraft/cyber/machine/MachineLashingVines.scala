...
import com.itszuvalex.femtocraft.cyber.tile.{TileCyberBase, TileLashingVines}
...
/**
  ...
  */
object MachineLashingVines {
  val NAME = "Lashing Vines"
}

class MachineLashingVines extends ICyberMachine {
  /**
    * @return Name of the machine
    */
  override def getName = MachineLashingVines.NAME
  ...
  override def multiblockRenderID: Int = RenderIDs.metabolicConverterID
  ...
  override def formAtBaseAndIndex(world: World, baseController: TileCyberBase, machineIndex: Int): Unit = {
    ...
    getTakenLocations(world, mx, my, mz).foreach { loc =>
      world.setBlock(loc.x, loc.y, loc.z, FemtoBlocks.blockLashingVines)
      world.getTileEntity(loc.x, loc.y, loc.z) match {
        case te: TileLashingVines =>
