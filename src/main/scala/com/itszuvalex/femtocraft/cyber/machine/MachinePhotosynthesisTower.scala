...
import com.itszuvalex.femtocraft.cyber.tile.{TileCyberBase, TilePhotosynthesisTower}
...
/**
  ...
  */
object MachinePhotosynthesisTower {
  val NAME = "Photosynthesis Tower"
}

class MachinePhotosynthesisTower extends ICyberMachine {
  /**
    * @return Name of the machine
    */
  override def getName = MachinePhotosynthesisTower.NAME

  override def getRequiredSlots = 4
  ...
  override def multiblockRenderID: Int = RenderIDs.photosynthesisTowerID
  ...
  override def formAtBaseAndIndex(world: World, baseController: TileCyberBase, machineIndex: Int): Unit = {
    ...
    getTakenLocations(world, mx, my, mz).foreach { loc =>
      world.setBlock(loc.x, loc.y, loc.z, FemtoBlocks.blockPhotosynthesisTower)
      world.getTileEntity(loc.x, loc.y, loc.z) match {
        case te: TilePhotosynthesisTower =>
